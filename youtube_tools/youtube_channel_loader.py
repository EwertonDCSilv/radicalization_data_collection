...
`dst_folder`: Specify an existing folder where the script will save several `.csv` files with the `captions`,
              `channel_id`, `comments`,`description`, `dislike_count`, `like_count`, `name`, `title`, `upload_date`,
              `video_id` and `view_count` for each of the videos in that channel (up to 2,500). Each file will be called
               `{video_id}.csv`.

 To obtain a channel Id sometimes you need to disambiguate a username.
 [You can do this online](https://commentpicker.com/youtube-channel-id.php).
"""
